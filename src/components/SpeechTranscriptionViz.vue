<template>
  <div class="speech-transcription-container">
    <div class="data-warning" v-if="detected_speech.length == 0">
      No speech data in JSON
    </div>

    <p
      class="speech"
      v-for="speech in indexed_speech"
      :class="{ current_speech: speech.current_speech }"
    >
      <span
        class="word"
        :class="{ current_word: word.current_word }"
        v-for="word in speech.words"
        @click="word_clicked(word)"
      >
        {{ word.word }}
      </span>
    </p>
      ...
      if (this.detected_speech) {
        this.detected_speech.forEach((element) => {
          if (element.alternatives[0].transcript)
            indexed_speech.push(
              new Detected_Speech(element, this.current_time)
            );
        });
      }
