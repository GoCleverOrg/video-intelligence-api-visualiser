<template>
  <div class="object-tracking-container">
    ...
    <div class="data-warning" v-if="face_tracks.length == 0">
      No face detection data in JSON
    </div>
    ...
    >
      ...
      <div class="segment-timeline">
        <div
          ...
          @click="segment_clicked(segment[0])"
        ></div>
      </div>
    </div>
    ...
    <transition-group name="segments" tag="div">
      <div
        class="face"
        v-for="face in indexed_face_tracks"
        @click="segment_clicked(face.start_time)"
        :key="face.thumbnail"
      >
        ...
        <div>
          <bar-chart
            :label="'Glasses'"
            :percent="face.attributes.glasses * 100"
          ></bar-chart>
          <!-- ... (other bar-charts for each attribute) -->
        </div>
      </div>
    </transition-group>
  </div>
      ...
      this.indexed_face_tracks.forEach((object_tracks) => {
        ...
        for (let index = 0; index < segments["face"].length; index++) {
          ...
          if (object_tracks.start_time < segment[1]) {
            segments["face"].segments[index][1] = Math.max(
              segments["face"].segments[index][1],
